...
differentiable estimator's MEFs match the truth with cosine $0.97$ and
RMSE $0.11$ (the caps-fixed F\&D-style baseline matches at cosine
$0.97$, RMSE $0.12$). For Jacobian validation we drop entries where
...
cross an active-set boundary). (b) Estimated vs.\ true MEFs; both the
full (navy) and caps-fixed F\&D (grey) estimators cluster around the
$y=x$ line with cosine $0.97$. A small number of estimated MEFs are
mildly negative: these are numerical artefacts from active-set
transitions where the autograd Jacobian sign-flips across a kink in
the merit-order map, not physical MEFs.}
