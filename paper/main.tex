...
recovers $f$ with cosine $0.905\pm0.019$, Spearman $0.909$, and
...
$6\%$ relative improvement with substantially tighter seed variance.
The warmstart variant matches KKT accuracy in $56\,$s vs.\ $343\,$s
for cold-start training (Sec.~\ref{sec:exp_warmstart}).
...
\caption{Recovery vs.\ training-set size $T$ (5 seeds; bands are
$\pm 1$ std).}
...
\caption{Recovery vs.\ observation noise $\sigma$ (5 seeds).}
...
estimate before gradient descent.  Fig.~\ref{fig:warmstart} shows that
\emph{diff.\ (warmstart)} reduces total training time by $6\times$
($56\,$s vs.\ $343\,$s) while matching the cold-start accuracy on clean
dispatch NRMSE ($0.220$ vs.\ $0.278$).  Fig.~\ref{fig:timing} places all
five methods on an accuracy--speed Pareto frontier: the warmstart variant
provides the best trade-off for time-constrained inference.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/warmstart.pdf}
\caption{Warmstart vs.\ cold-start: (a) clean dispatch NRMSE;
(b) wall-clock training time (5 seeds, 95\% CI).}
\label{fig:warmstart}
\end{figure}
...
\caption{Accuracy--speed trade-off across all methods (error bars are
95\% CI over seeds).}
\label{fig:timing}
\end{figure}
...
in the Huber-like dead-zone loss.  Fig.~\ref{fig:epsilon_sweep} shows
...
($\varepsilon=0$) and overly aggressive margins that ignore small
residuals.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/epsilon_sweep.pdf}
\caption{Effect of insensitivity margin $\varepsilon$ on recovery
(95\% CI bands).}
...
Fig.~\ref{fig:counterfactual} shows that relative CO${}_2$ error falls
from $2.0$ at extrapolation scale $0.8$ to $0.07$ near the in-sample
scale $1.0$, and rises symmetrically for $1.2\times$ demand.  This
validates the model as a practical tool for counterfactual energy-policy
analysis when operated near its training distribution.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/counterfactual.pdf}
\caption{Counterfactual CO${}_2$ analysis: (a) relative error vs.\ demand
scale; (b) true vs.\ estimated CO${}_2$ scatter (colour = demand scale).}
\label{fig:counterfactual}
\end{figure}

\subsection{Marginal emission factor recovery}
\label{sec:exp_mef}
Fig.~\ref{fig:mef_exp} plots the per-seed MEF point estimates alongside a
bar chart of relative MEF error.  The differentiable estimator achieves a
mean relative error below $0.08$ across seeds, confirming that the
...
\caption{MEF recovery: (a) true vs.\ estimated MEF per seed;
(b) relative MEF error (95\% CI).}
\label{fig:mef_exp}
\end{figure}
